...
class CardCollectionDirective(Directive):
    ...
    def run(self):
        ...
        if 'title' in options :
            bootstrapContainer += nodes.raw(text='<h2 style="margin-bottom:5px;">{0}</h2>'.format(options['title']), format='html')
            if 'subtitle' in options :
                if 'subtitle_link' in options :
                    bootstrapContainer += nodes.raw(text='<p><a href="{0}" class="text-muted" style="text-decoration:none;">{1}</a><br /></p>'.format(options['subtitle_link'], options['subtitle']), format='html')
                else :
                    bootstrapContainer += nodes.raw(text='<p class="text-muted">{0}<br /></p>'.format(options['subtitle']), format='html')
            else :
